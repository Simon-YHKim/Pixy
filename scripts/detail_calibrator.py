...
function compose(){var r=+document.getElementById('r_resolution').value,c=+document.getElementById('r_colors').value,
  ...
  // dialed numbers -> the exact spec command (resolution -> canvas/scale)
  var res=STEPS.resolution[idx('resolution')];
  var sc=Math.max(1,Math.round(512/res));
  p+='\n\n# Lock the dialed target as a project spec:\npython scripts/init_spec.py --out pixy.spec.json --canvas '+res+'x'+res+' --scale '+sc;
